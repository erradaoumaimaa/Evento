<x-dashboard.layoutCompany class="flex flex-col space-y-8">
    ...
    <section class="flex flex-col items-center my-10 py-10 px-32 bg-gray-50">
        @if($jobs->isNotEmpty())
            <h1 class="text-2xl font-black p-4 border-4 border-dashed border-white text-black bg-jaune rounded-lg py-4 px-16 text-center z-[20] translate-y-[50%]">Your Post Jobs</h1>
            <div class="flex flex-row flex-wrap w-full border-4 border-dashed border-jaune rounded-lg p-8 ">
                @foreach($jobs as $job)
                    <!-- Card Component -->
                    <div class="transition-all duration-150 flex w-full px-4 py-6 md:w-1/2 lg:w-1/3">
                        <div class="flex flex-col items-stretch min-h-full pb-4 mb-6 transition-all duration-150 bg-white rounded-lg shadow-lg hover:shadow-2xl">
                            <div class="md:flex-shrink-0">
                                <img src="{{ asset('images/hiring.png') }}" alt="Job Cover" class="object-fill w-full rounded-lg rounded-b-none md:h-56" />
                            </div>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center flex-1">
                                        ...
                                        <div class="flex flex-col mx-2">
                                            <a href="#" class="font-semibold text-black-700">{{ $job->company->user->name}}</a>
                                            <p class="text-gray-700">in <span class="text-xs text-green-700">{{ $job->company->industry }}</span></p>
                                        </div>

                                    </div>
                                    <div>
                                        <button class="bg-green-700 text-white font-medium px-4 py-2 rounded-md flex gap-1 items-center">Read more <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                        </svg>
                                        </button>
                                        <a href="{{ route('jobs.edit', $job->id) }}"> Update
                                        </a>
